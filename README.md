...
### Semana 06 - Processamento HTTP e Formulários Web
...
<form action="processar.php" method="POST">
    ...
    nome="nomeUsuario" placeholder="Digite seu Nome">
...
* **Método GET**: Solicitar informações públicas e realizr buscas, mas altamente arriscado para dados privados.
* **Método POST**: As informações viajam guardadas dentro do protocolo. 

#### Testar o uso dos Protocoos HTTP
...
- **$_GET**: Armazena dados passados pelo URL via parêmetros de consulta(query scring);
- **$_POST**: Recolhe dados enviados para formulários usando método HTTP POST.
- **$_SERVER**: Contém informações sobre o servidor, ambiente e caminho de **script**.

**Porque usar `??` (Dupla interogação) para obter dados de SuperGlobais**?
...
Na primeira vez que uma página é aberta, o fomulário ainda não foi enviada. Portanto, a chave pode não existir no `array`.
...
>Obs: Use htmlspecialchars() ao exibir valor em HTML => Converte caracteres especiais em entidades correspondentes em HTML, evitando que o código seja interpretado erradamente pelo navegador. É usado principalmente na segurança web para evitar ataques Cros-Site-Scripting(XSS)

#### Validação de Dados no BackEnd é Obrigatória.

Muitos esenvolvedore iniciantes acreditam que colocar atributos `require`, `type=email` ou `min=0` na <tag> do HTML é suficiente para protegrer o sistema. **Isso é uma ilusão!**. Sempre fazer as validações de dados no código BackEnd
...
A técnica do **Stick Form** consiste em imprimir de volta no atributo "value" no input os dados que o usuário acaba de digitar caso ocorra um erro de validação de dados.
...
```
---
### Semana 7 - Segurança do BackEnd - Sanitização, Validação e Proteção contra XSS

#### 1º Andamento do Desenvolvedor BackEnd - 

> Nunca Confie no Usuário: Toda entrada de dados vindo de fora do servidor é potenciamente maliciosa até que seja riforosamente validada, sanitizada e codificada.

Quando você disponibiliza um campo de texto em um site, qualquer pessoa conectada a internet pode digitar códigos maliciosos em vez de texto.
Se o código BackEnd pega esse texto diretamente sem nenhum tratamento, a ordem de execução de código abrirá porta para a invasão devastadoras do seu sistema.


#### A Anatomia de um Ataque: O que é Cross-Site Scripting (XSS)

o XSS ocorre quando uma aplicação web inclui dadsos não confiaveis em uma página web sem a devida validação ou escape caracteres. Isso permite que u, atacante execute Scripts maliciosos (geralmente em JavaScript) diretamente no navegador de outro usuário que visitam o site.

**Como o Ataque Acontece:**
1. *Roubo de Sessão(Cookie Stealing)*: O JavaScript injeta lê os cookies de autenticação da vítima (documente.cookie) e os envia para o servidor de atacante, permitendo que ele faça login na conta da vítima sem precisar de senha.
...
3. *Redirencionamento Malicioso*: Força o navegador da vítima a abrir sites com vírus ou páginas clinadas de banco.

4. *Captura de Teclas(Keylagger)*: Grava tudo o queb a vítima digita enquanto a página estiver aberta.
...
#### **As Tríagem de Defesa: Validaçãp, Sanitização e Escapamento**
...
A função `htmlspecialchars()` é o principal mecanismo de PHP para neutralizar XSS na camada de apresentação
...
```
- **`ENT_QUOTES`**: Converte tatos aspas simples.
Essencial para saídas em atributos HTML
- **`ENT_SUBSTATUTE`**: Substatui sequências de bytes inválidos por caracteres de substituição Unicode em vez de retomar uma string vazi
...
Para não precisar digitar essa linha extensa em todas as partes de saáda de texto para HTML, os desenvolvedores profissionais criam uma função auxilial curta:

```php
function e(string $texto):string {
    return htmlspecialchars($texto, ENT_QUOTES | ENT_SUBATITUTE | ENT_HTML5, "UFT-8")
}

<p>Comentário: <?= e($comentariosUsuario)?></p>
<input type="text" name="nome" value="<? e($nomeUsuario) ?>"/>
```

#### **Validação e Sanitização com `filter_var()`**

O PHP possui a biblioteca de filtros nativos `filter_var()`. Observe os filtros mais importantes do ecossistema coporativo:
...
#### **1. Da Memória Volátil ao Bnaco de Dados**

Em sistemas corporativos de grande porte, arquivos planos (`.txt` `.json`) não oferece a segurança, integridade, concorrencia e velocidade necessária para armazenamento de dados. Então é aqui que o **BackEnd** encontra o **Banco de Dados Relacionl**.
...
> Obs: ACID:
> Atomicidade, assegura que cada transaçãoseja unica.
> Consistêcia, respeita todas as regras, restrições e chaves definidas, garantindo a validade da transação.
...
- **Tratamento Orientado a Objetos com Exception**: Em vez de retronar código de erros, o PDO lamça uma instancia da classe especializada `PDOExcepton`.
...
- PDO::ATTREMULATE_PREPARES => false : Garante que o PHP envie a consulta e os parêmetros separados diretamente para o planejador do BD processar, blindando e aplicação contra ataques sofisticados de `SQL_injection`
...
3. A empresa é penalizada por violação da **LGOD(Lei Geral de Proteção de Dados)**
